...
SDRPP_MOD_INFO{
    /* Name:            */ "usrp_b2xx_source",
    /* Description:     */ "USRP B2XX source module for SDR++",
    /* Author:          */ "Ryzerth",
    /* Version:         */ 0, 1, 0,
    /* Max instances    */ 1
};
...
public:
    USRPSourceModule(std::string name) {
        ...
        sigpath::sourceManager.registerSource("USRP B2XX", &handler);
    }

    ~USRPSourceModule() {
        stop(this);
        sigpath::sourceManager.unregisterSource("USRP B2XX");
    }
    ...
    void selectChannel(uhd::usrp::multi_usrp::sptr dev, std::string chan) {
        ...
        if (config.conf["devices"][selectedSer].contains("channels") && config.conf["devices"][selectedSer]["channels"].contains(selectedChan)) {
            ...
            if (cconf.contains("clockrate")) {
                clockRate = cconf["clockrate"];
            }
        }
        config.release();
        dev->set_master_clock_rate(clockRate);
...
private:
    ...
    static void menuHandler(void* ctx) {
        ...
        SmGui::LeftLabel("Clock rate");
        SmGui::FillWidth();
        SmGui::ForceSync();
        if (SmGui::InputInt(CONCAT("##_usrp_cr_", _this->name), &_this->clockRate, 1000000, 10000000)) {
            config.acquire();
            config.conf["devices"][_this->selectedSer]["channels"][_this->selectedChan]["clockrate"] = _this->clockRate;
            config.release(true);
        }
...
MOD_EXPORT void _INIT_() {
    ...
    def["devices"] = json({});
    def["device"] = "";
    config.setPath(core::args["root"].s() + "/usrp_b2xx_config.json");
    config.load(def);
